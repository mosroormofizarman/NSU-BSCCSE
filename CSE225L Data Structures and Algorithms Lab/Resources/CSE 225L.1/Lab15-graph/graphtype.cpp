...
template<class VertexType> GraphType<VertexType>::~GraphType()
{
    delete [] vertices;
    delete [] marks;
    for(int i=0; i<maxVertices; i++) delete [] edges[i];
    delete [] edges;
}
...
void GraphType<VertexType>::BreadthFirstSearch(VertexType startVertex, VertexType endVertex)
{
    ...
    while(!stack.IsEmpty())
    {
        ...
        stack.Pop();
        v2 = stack.Top();
        if(FoundEdge(v1,v2) && !stack.IsEmpty()){
            l+=1;
            stack.Pop();
        }
    }
    cout << endl;
    cout<<"length of shortest path:"<<l+1<<endl;
    if (!found)
        cout << "Path not found." << endl;
}

template<class VertexType>
bool GraphType<VertexType>::FoundEdge(VertexType a, VertexType b)
{
    if(WeightIs(a,b)==NULL_EDGE)
        return false;
    return true;
}
